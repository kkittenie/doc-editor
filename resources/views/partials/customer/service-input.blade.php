<div class="md:col-span-2 xl:col-span-4">
    <div class="flex items-center justify-between">
        <p class="text-xs font-semibold text-slate-warm-600 dark:text-parchment-300">
            Service <span class="font-normal text-slate-warm-400"></span>
        </p>
        ...
        <table class="w-full min-w-[640px] text-left text-xs">
            <thead>
                <tr class="bg-parchment-100 text-slate-warm-500 dark:bg-slate-warm-800 dark:text-parchment-400">
                    ...
                    <th class="w-40 px-3 py-2 font-semibold">Harga (Rp)</th>
                    <th class="w-36 px-3 py-2 font-semibold">Tipe</th>
                    <th class="w-12 px-3 py-2"></th>
                </tr>
            </thead>

            <tbody>
                <template x-for="(row, index) in serviceRows" :key="'s' + index">
                    <tr class="border-t border-parchment-200 dark:border-slate-warm-800">
                        <td class="px-3 py-2">
                            <input type="text" :name="'services[' + index + '][name]'"
                                x-model="row.name" maxlength="150" placeholder="Contoh: Internet 100 Mbps"
                        ...
                        <td class="px-3 py-2">
                            <input type="number" min="0" step="0.01" :name="'services[' + index + '][price]'"
                                x-model="row.price" placeholder="0"
                                class="h-9 w-full rounded-lg border border-parchment-300 bg-white px-2.5 text-xs outline-none focus:border-ink-900 dark:border-slate-warm-700 dark:bg-slate-warm-900">
                        </td>

                        <td class="px-3 py-2">
                            <select :name="'services[' + index + '][price_type]'" x-model="row.price_type"
