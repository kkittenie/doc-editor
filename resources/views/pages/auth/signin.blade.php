...
@section('content')
<div
    class="min-h-screen flex items-center justify-center p-6 bg-parchment-100 dark:bg-slate-warm-950 relative overflow-hidden">
    ...
    <div class="w-full max-w-[400px] relative z-10">
        ...
        <div
            class="mb-6 p-3 rounded-lg border border-parchment-300 bg-parchment-50 text-ink-800 text-xs font-medium flex items-center gap-2 dark:bg-slate-warm-900 dark:border-slate-warm-700 dark:text-parchment-200">
        ...
        <div
            ...
            <form action="{{ route('signin') }}" method="POST">
                @csrf
                <div>
                    <label for="signin-login"
                        class="block text-xs font-semibold text-ink-900 dark:text-parchment-200 mb-1.5">Email atau
                        Username</label>
                    <input id="signin-login" type="text" name="login" required
                        placeholder="nama@perusahaan.co.id atau username" value="{{ old('login') }}"
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="signin-password"
                            class="block text-xs font-semibold text-ink-900 dark:text-parchment-200">Kata Sandi</label>
                        <a href="#"
                            class="text-xs text-bronze-700 hover:text-bronze-900 dark:text-bronze-400 dark:hover:text-bronze-300 transition-colors">Lupa
                            sandi?</a>
                    </div>
                    <div x-data="{ show: false }" class="relative">
                        <input id="signin-password" :type="show ? 'text' : 'password'" name="password" required
                ...
                <div class="flex items-center gap-2.5">
                    <input type="checkbox" id="remember" name="remember"
                        class="rounded border-parchment-400 text-ink-900 focus:ring-ink-900/20 dark:border-slate-warm-600 dark:bg-slate-warm-700" />
                    <label for="remember" class="text-xs text-slate-warm-600 dark:text-parchment-300 select-none">Ingat
                        saya di perangkat ini</label>
                </div>

                <button type="submit"
                    class="w-full rounded-lg bg-ink-900 text-parchment-50 py-3 text-sm font-semibold shadow-md hover:bg-ink-800 active:scale-[0.98] transition-all dark:bg-parchment-100 dark:text-ink-900 dark:hover:bg-parchment-200">
                    Masuk
                </button>
            </form>
        </div>
        ...
        <p class="text-center text-xs text-slate-warm-500 dark:text-parchment-400 mt-6">
            Belum punya akun?
            <a href="/signup" class="font-semibold text-ink-900 hover:underline dark:text-parchment-200">Buat akun
                baru</a>
        </p>
